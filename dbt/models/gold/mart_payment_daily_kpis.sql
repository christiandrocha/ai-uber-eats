...
SELECT
    ...
    COUNT_IF(captured_at IS NOT NULL)                                              AS captured_payments,
    COUNT_IF(authorized_at IS NOT NULL AND captured_at IS NULL)                    AS authorized_payments,
    COUNT_IF(authorized_at IS NULL)                                                AS pending_payments,
    ROUND(COUNT_IF(captured_at IS NOT NULL) * 100.0 / COUNT(payment_id), 2)       AS capture_rate_pct,
